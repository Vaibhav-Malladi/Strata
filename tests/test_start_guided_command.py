...
def test_non_complete_output_contains_exactly_one_next_line():
    output = start_command.build_start_command_output(_guided_view())

    assert _next_line_count(output) == 1


def test_completed_output_contains_no_next_line():
    ...
    assert _next_line_count(output) == 0
...
def test_apply_next_action_displays_confirmation_guidance():
    ...
    assert "Next: Apply the reviewed changes" in output
...
def test_no_rich_panel_or_progress_bar_dependency_is_introduced():
    source = inspect.getsource(start_command)

    for forbidden in ("rich", "Progress", "Panel", "print_status_card", "status_spinner"):
        assert forbidden not in source
...
def _next_line_count(output: str) -> int:
    return sum(1 for line in output.splitlines() if line.startswith("Next:"))
...
TESTS = [
    ...
    test_non_complete_output_contains_exactly_one_next_line,
    test_completed_output_contains_no_next_line,
    ...
    test_no_rich_panel_or_progress_bar_dependency_is_introduced,
